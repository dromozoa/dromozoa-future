...
assert(future_service():dispatch(function (service)
  ...
  local f2 = service:deferred(function (promise)
    local reader = shared_reader:share()
    print("f2", coroutine.running())
    reader:read(1):get()
    print("f2 done")
    return promise:set(true)
  end)

  print("f1", f1, f1.state)
  print("f2", f2, f2.state)
  ...
  print("f1", f1.state, f1.state.status)
  print(f1:get())
  print("f2", f2.state, f2.state.status)
  print(f2:get())
